...
struct OpenModelicaGeneratedFunctionCallbacks {
  ...
  */
  ...
  /*
  * FMU adjoint (vector-Jacobian product) partial derivative functions, used by
  * fmi3GetAdjointDerivative. Only generated by the new backend, which is the
  * only backend that produces the symbolic adjoint (ADJ) Jacobian required
  * here; the old backend leaves initialPartialFMIDERADJ NULL.
  */
  initialAnalyticalJacobian_func_ptr initialPartialFMIDERADJ;
  jacobianColumn_func_ptr functionJacFMIDERADJ_column;
  const int INDEX_JAC_FMIDERADJ;
};
